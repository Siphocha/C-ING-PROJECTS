...
//Load the students data from the file. Easiest to do.
void loadStudents() {
    FILE *file = fopen(FILENAME, "rb");
    if (file == NULL) {
        return;
    }
    //Defining how the function interpretes studentsCount
    studentsCount = fread(students, sizeof(Student), MAX_STUDENTS, file);
    fclose(file);
}

void saveStudents() {
    //Begin with error when students can't be saved
...
void addStudent() {
    if (studentsCount >= MAX_STUDENTS) {
        //Have to error check for the maximum amount
        printf("Maximum number of students, no more!\n");
        return;
    }

    //Under the Student data struc using to define what a newStudent is
    Student newStudent;
    ...
    //Check if "newly" estabilshed student is already registered
...
void displayStudents() {
    //Just incase theres no students to display
    if (studentsCount == 0) {
    ...
    for (int i = 0; i < studentsCount; i++) {
        printf("%-20s %-10d %-5d %-15s %-10.2f\n",
               ...
               students[i].course, students[i].average);
    }
}
...
void updateStudent() {
    //Update the students info when it's needed
    if (studentsCount == 0) {
        printf("No students yet\n");
        return;
    }
    ...
    switch(field) {
        ...
        case 3:
            ...
            fgets(students[found].course, MAX_COURSE_LENGTH, stdin);
            students[found].course[strcspn(students[found].course, "\n")] = '\0';
...
void deleteStudent() {
    ...
    if (found == -1) {
        //Catching an extremely unlikely edge case here
    ...
    if (tolower(confirm) == 'y') {
        // Shift all elements after the found index left by one
